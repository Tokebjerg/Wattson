...
# --------------------------------------------------------------------------- #
def test_solar_ev_smoothing():
    """P1-P4: solar_only must stop coasting on grid through a sustained cloud
    collapse, throttle DOWN fast, and keep a higher in-session floor — without
    flapping or starving the car on clean sun. Pure helpers + an end-to-end
    broken-cloud EV-loop sim (old vs new) measuring grid-import-under-the-solar-label."""
    from datetime import datetime, timedelta, timezone
    checks = []
    TZ = timezone(timedelta(hours=2))
    t0 = datetime(2026, 6, 17, 10, 0, tzinfo=TZ)

    # --- P2 pure helper: robust_surplus_signal = min(mean_120s, median_45s) ----- #
    med = planner._median
    checks.append(("median of odd/even lists", med([5, 1, 3]) == 3 and med([4, 2]) == 3.0, f"{med([5,1,3])}/{med([4,2])}"))

    def samples(vals, step=10):
        # vals = list of surplus W, one per `step` seconds ending at t0
        n = len(vals)
        return [(t0 - timedelta(seconds=(n - 1 - i) * step), float(v)) for i, v in enumerate(vals)]

    rs = planner.robust_surplus_signal
    # Steady high sun -> signal ~= the level (mean==median), no starvation.
    steady = rs(samples([3000] * 15), t0, avg_seconds=120, fast_seconds=45)
    checks.append((f"steady sun: signal tracks the level (got {steady:.0f})", abs(steady - 3000) < 1, f"{steady:.0f}"))
    # Collapse: last 45s are ~0 after a long high run -> FAST DOWN (median pulls it down).
    collapsed = rs(samples([4000] * 9 + [0, 0, 0, 0, 0, 0]), t0, avg_seconds=120, fast_seconds=45)
    checks.append((f"collapse: signal drops fast via the 45s median (got {collapsed:.0f})", collapsed < 200, f"{collapsed:.0f}"))
    # Single-sample dip (car cycling) must NOT collapse the signal (median is robust).
    blip = rs(samples([3000, 3000, 0, 3000, 3000, 3000]), t0, avg_seconds=120, fast_seconds=45)
    checks.append((f"single 0-sample dip does NOT starve the car (median robust, got {blip:.0f})", blip > 1500, f"{blip:.0f}"))
    # Slow up: a fresh spike after a low run is capped by the slow mean (anti-flap).
    ramp = rs(samples([200] * 9 + [5000] * 5), t0, avg_seconds=120, fast_seconds=45)
    checks.append((f"up-ramp capped by the slow mean, not the fast median (got {ramp:.0f})", ramp < 2500, f"{ramp:.0f}"))

    # --- P3: in-session floor closes the [840,1410) grid-leak band --------------- #
    def ev_state(surplus_override, drawing):
        return models.SiteState(
            timestamp=t0, pv_power_w=0.0, load_power_w=0.0, load_includes_ev=True,
            grid_power_w=0.0, grid_import_power_w=0.0, grid_export_power_w=0.0, battery_soc_pct=100.0,
            battery_power_w=0.0, inverter_online=True, inverter_status="normal", easee_online=True,
            easee_status="charging" if drawing else "disconnected",
            easee_power_w=1400.0 if drawing else 0.0, easee_session_kwh=0.0, easee_phase_mode="auto",
            current_buy_price=0.4, current_sell_price=0.5, forecast_today_kwh=0.0, price_slots=[], solar_slots=[])

    def solar_plan(surplus, drawing):
        return planner.build_ev_plan(
            ev_state(surplus, drawing), ev_mode=const.EV_MODE_SOLAR_ONLY, ev_max_amps=16,
            ev_solar_min_surplus_w=1400, ev_windows="", solar_surplus_override=float(surplus))

    # Active session, surplus 1000W (in the OLD [840,1410) leak band): a 6A offer
    # draws 1410W, so >=410W would be grid -> must now PAUSE instead.
    p_leak = solar_plan(1000, drawing=True)
    checks.append((f"in-session 1000W surplus PAUSES (was grid-leak at 840 floor): {p_leak.desired_action}",
                   p_leak.desired_action == "pause", p_leak.desired_action))
    # Active session, surplus 1300W (> stop 1260, < start 1400): hysteresis keeps it charging.
    p_hyst = solar_plan(1300, drawing=True)
    checks.append((f"in-session 1300W surplus keeps charging (hysteresis above stop): {p_hyst.desired_action}",
                   p_hyst.desired_action == "resume", p_hyst.desired_action))
    # Not yet active: still needs the full start threshold (1400) — 1300W does NOT start.
    p_start = solar_plan(1300, drawing=False)
    checks.append((f"not-yet-charging 1300W does NOT start (start>stop): {p_start.desired_action}",
                   p_start.desired_action == "pause", p_start.desired_action))

    # --- End-to-end broken-cloud EV-loop sim: old vs new grid-import ------------- #
    # 10s steps. Replicates the solar_only resume/amps + dip-hold + apply-gate so
    # the ONLY differences are the four levers (signal, P3 floor, dip-hold release,
    # down-fast apply). House 700W; 1-phase regime (surplus stays <4430W).
    HOUSE = 700.0
    def pv_profile():
        seq = []
        seq += [3500] * 18   # 0-180s  bright  (surplus 2800 -> ~11A)
        seq += [250] * 24    # 180-420s SUSTAINED collapse (4 min) — the test
        seq += [3500] * 12   # 420-600s bright
        seq += [250] * 6     # 600-660s BRIEF dip (60s) — should ride, not pause
        seq += [3500] * 30   # 660-960s bright
        return seq

    def run(pv_seq, mode):
        samples_l: list = []
        offered = 0; last_amps = 0; last_change = -999; hold_until = -999
        import_since = None; action = "pause"
        grid_imp_ev = 0.0; ev_kwh = 0.0; amp_changes = 0; pauses = 0; dt = 10
        stop_w = 1260.0 if mode == "new" else 840.0
        for i, pv in enumerate(pv_seq):
            t = i * dt
            ev_draw = offered * 235.0 if action == "resume" else 0.0
            surplus = max(0.0, pv - HOUSE)
            samples_l.append((t, surplus))
            samples_l = [(tt, v) for (tt, v) in samples_l if tt >= t - 120]
            if mode == "new":
                avg = sum(v for _, v in samples_l) / len(samples_l)
                fast = [v for tt, v in samples_l if tt >= t - 45]
                signal = min(avg, sorted(fast)[len(fast) // 2] if len(fast) % 2 else (sorted(fast)[len(fast)//2-1]+sorted(fast)[len(fast)//2])/2)
            else:
                signal = sum(v for _, v in samples_l) / len(samples_l)
            session = ev_draw > 200 or action == "resume"
            req = stop_w if session else 1400.0
            want_action = "resume" if signal >= req else "pause"
            want_amps = max(6, min(int(signal // 235), 16)) if want_action == "resume" else 0
            # dip-hold
            if want_action == "resume":
                hold_until = t + 180; import_since = None
            elif want_action == "pause" and session and t < hold_until:
                grid_if_held = HOUSE + (last_amps * 235.0) - pv
                if mode == "new":
                    importing = max(0.0, grid_if_held) > 300.0
                    if importing:
                        import_since = t if import_since is None else import_since
                        held = t - import_since
                    else:
                        import_since = None; held = 0
                    if not (importing and held >= 60):
                        want_action, want_amps = "resume", last_amps  # ride brief dip
                else:
                    want_action, want_amps = "resume", last_amps      # old: always ride
            # apply-gate
            struct = want_action != action
            within_db = abs(want_amps - last_amps) < 2
            retune_due = (t - last_change) >= 90
            down = want_amps < last_amps
            change = (not within_db) and (retune_due or (down and mode == "new"))
            if struct:
                action = want_action
                if action == "pause":
                    offered = 0; pauses += 1
                else:
                    offered = want_amps; last_amps = want_amps; last_change = t; amp_changes += 1
            elif change:
                offered = want_amps; last_amps = want_amps; last_change = t; amp_changes += 1
            ev_draw_real = offered * 235.0 if action == "resume" else 0.0
            grid = HOUSE + ev_draw_real - pv
            if ev_draw_real > 0:
                grid_imp_ev += max(0.0, grid) / 1000.0 * dt / 3600.0
                ev_kwh += ev_draw_real / 1000.0 * dt / 3600.0
        return {"grid": grid_imp_ev, "ev": ev_kwh, "amp_changes": amp_changes, "pauses": pauses}

    seq = pv_profile()
    old = run(seq, "old")
    new = run(seq, "new")
    checks.append((f"broken-cloud: NEW cuts grid-import-under-solar >50% (old {old['grid']*1000:.0f}Wh -> new {new['grid']*1000:.0f}Wh)",
                   new["grid"] < old["grid"] * 0.5, f"old={old['grid']*1000:.0f} new={new['grid']*1000:.0f} Wh"))
    # NEW SHOULD deliver less total than OLD (it stops grid-charging the collapse),
    # but it must still capture the bright-period SOLAR (~0.43 kWh available) — not
    # starve. And its grid fraction must be far lower than OLD's.
    old_frac = old["grid"] / max(0.01, old["ev"]); new_frac = new["grid"] / max(0.01, new["ev"])
    checks.append((f"broken-cloud: NEW still captures the bright-period solar (not starved): {new['ev']:.2f} kWh",
                   new["ev"] > 0.35, f"new={new['ev']:.2f} kWh"))
    checks.append((f"broken-cloud: NEW grid-fraction clearly below OLD ({new_frac*100:.0f}% vs {old_frac*100:.0f}%)",
                   new_frac < old_frac * 0.7, f"new={new_frac*100:.0f}% old={old_frac*100:.0f}%"))

    # Clean all-day sun: NEW must NOT pause or flap (no false collapse detection).
    # 90 steps x 10s = 900s at ~11A 1-phase (~2.6 kW) -> ~0.65 kWh.
    clean = run([3500] * 90, "new")
    checks.append((f"clean sun: NEW charges steadily, no false pauses (pauses={clean['pauses']}, ev={clean['ev']:.2f})",
                   clean["pauses"] == 0 and clean["ev"] > 0.5, f"pauses={clean['pauses']} ev={clean['ev']:.2f}"))
...
def main():
    ...
    for title, suite in (("PHASE A · A1 HORIZON INGESTION", test_horizon),
                         ...
                         ("SOLAR_ONLY EV SMOOTHING (P1-P4: no grid-coast / fast-down / floor)", test_solar_ev_smoothing),
