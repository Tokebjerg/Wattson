...
# ---------------------------------------------------------------------------
# EXPERIMENT (2026-06-17, user-directed, v0.24.13) — MORNING SELL-THROTTLE
# ---------------------------------------------------------------------------
# Hypothesis under test: throttling the battery charge register to a LOW value
# during the morning while solar_sell is ON makes the pack fill slowly so the
# surplus EXPORTS instead — deferring the bulk fill to the cheaper/negative
# midday hours (the SunMate / kellerza-sunsynk / solarenergyconcepts technique).
#
# This INTENTIONALLY overrides floor_sell_safe inside the window. CAUTION: on
# this klatremis/Deye build, charge<=10 A + solar_sell=ON is the exact register
# pair that stalled the MPPT in v0.23.0 — BUT every one of those observations was
# during rapid sell/charge FLAPPING (sell toggling ~every 2 min). The community
# uses a STABLE low setpoint successfully, so this is applied as a STEADY value
# across the whole window (no flapping) to test whether a stable low charge
# exports cleanly. If PV collapses in the morning, THIS is the cause; the window
# self-heals to the full rate at MORNING_SELL_THROTTLE_END_HOUR. Evaluate live.
MORNING_SELL_THROTTLE_A: float = 10.0
MORNING_SELL_THROTTLE_START_HOUR: int = 7
MORNING_SELL_THROTTLE_END_HOUR: int = 11  # exclusive — full rate resumes at 11:00


def apply_morning_sell_throttle(
    plan,
    *,
    hour: int,
    soc_pct: float,
    max_soc_pct: float,
    throttle_a: float = MORNING_SELL_THROTTLE_A,
    start_hour: int = MORNING_SELL_THROTTLE_START_HOUR,
    end_hour: int = MORNING_SELL_THROTTLE_END_HOUR,
) -> "object":
    """Throttle the charge register to ``throttle_a`` when, in the morning window,
    the plan is SELLING surplus and the pack has room. Runs AFTER floor_sell_safe
    and deliberately undoes its floor for this window only. No-op when not selling,
    outside the window, or at a full battery — so the low charge can only ever ride
    WITH an active sell (the configuration under test), never on its own.
    """
    if (
        getattr(plan, "desired_solar_sell", False)
        and start_hour <= hour < end_hour
        and soc_pct < max_soc_pct
    ):
        return replace(
            plan,
            desired_max_charge_current_a=float(throttle_a),
            reason=(
                f"{plan.reason} | morning sell-throttle {throttle_a:.0f} A "
                "(deferring bulk fill to the cheaper midday — EXPERIMENT)"
            ),
        )
    return plan
