...
**v0.24.9 — 2026-06-17 — "REN SOL" EV-LADNING: stop net-import maskeret som sol (bruger: "ren sol kører ikke optimalt, mistænker curtailment" → evaluér → "udfør, simuler og deploy"):** En 18-agent-workflow på dagens RIGTIGE ladedata (09:40-11:19) viste: det er IKKE curtailment (curtailment-sensoren 0,03 kWh er KORREKT; ~0,7 kWh overskud blev lovligt SOLGT). Det reelle problem: **~30-40% af EV-energien (≈1,5-1,8 kWh) kom fra NETTET maskeret som "Ren sol"**, værre efter batteriet blev fuldt (23%→54%). `reason_for_no_current="limited_by_charger_dynamic_limit"` bekræftede at Wattsons tilbudte ampere bandt. (Min foreløbige fase-lås-teori blev AFKRÆFTET af verifikationen — fase-låsen fyrer aldrig i solar_only, som altid kommanderer `auto_phase`; alle læk-øjeblikke var 1-faset 6A.) **Fire fixes implementeret: P4 (diagnostik):** ny `sensor.*_ev_solar_grid_today` = ∫min(EV-træk, net-import) i Ren sol — gør det usynlige læk målbart (`grid_fraction_pct`). **P3 (planner):** in-session-fortsæt-gulvet hævet fra 840 W til `EV_SOLAR_SESSION_STOP_W=1260` (lige under det reelle 6A-1-fase-træk på 1410 W) med start>stop-hysterese — lukker det strukturelle [840,1410) net-læk-bånd. **P2 (coordinator):** asymmetrisk surplus-signal `min(120s-middel, 45s-MEDIAN)` (`robust_surplus_signal`, pure) — hurtig NED ved kollaps, langsom OP (anti-flap); medianen er robust mod bilens egen 0↔11kW-cykling i surplus-estimatet (ren MIN ville sulte bilen). Plus: materielle NED-flytninger af tilbudt strøm bypasser 90s-retune i apply-gaten. **P1 (coordinator):** 3-min solar-dip-holdet frigives nu udfaldsbaseret — hvis måleren viser net-import >300W mens holdet hævder "sol" i >60s, slippes pausen igennem (i stedet for at coaste på nettet gennem et flere-minutters sky-kollaps). **Validering:** ny `test_solar_ev_smoothing` med broken-cloud EV-loop-sim (spike→kollaps→spike): grid-import-under-sol skåret **209→98 Wh (>50%)**, grid-fraktion 41%→24%, INGEN falske pauser eller sult på ren sol. Sim **328/328** (+12). Afkræftet/sprunget over (genbesøg ikke): fase-lås-stranding, batteri-destabilisering, ±1A-slew (modsiger deadbandet), P5 amp-hysterese (neutraliseret af P1/P2). Ingen hård invariant rørt; bil-agnostisk, deadline-backup + batteri-prioritet-gate urørt.
